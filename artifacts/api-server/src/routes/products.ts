...
router.get("/", optionalAuth, async (req, res) => {
  try {
    const { search, category, subcategory, game, sort = "newest", page = "1", limit = "20" } = req.query as any;
    ...
    if (game) conditions.push(eq(products.game, game));
    if (search) {
...
router.post("/", authMiddleware, limits.createProduct, async (req, res) => {
  try {
    ...
    if (trimmedTitle.length < 3 || trimmedTitle.length > 200) {
      res.status(400).json({ message: "Название должно быть от 3 до 200 символов" });
      return;
    }
    if (trimmedDesc.length < 10 || trimmedDesc.length > 5000) {
      res.status(400).json({ message: "Описание должно быть от 10 до 5000 символов" });
      return;
    }
    const parsedPrice = parseFloat(price);
    if (!isFinite(parsedPrice) || parsedPrice < 1 || parsedPrice > 1_000_000) {
      res.status(400).json({ message: "Цена должна быть от 1 до 1 000 000 ₽" });
      return;
    }
    if (images && images.length > 10) {
      res.status(400).json({ message: "Максимум 10 изображений" });
      return;
    }

    const finalImages: string[] = (images && images.length > 0)
      ? images
      : [generatePlaceholderImage({ title: trimmedTitle, category, subcategory, game })];
